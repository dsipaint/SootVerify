...
public class StopListener extends ListenerAdapter
{
	public void onGuildMessageReceived(GuildMessageReceivedEvent e)
	{
		if(e.getMember() != null && isStaff(e.getMember()))
		{
			...
			if(args.length > 1 && args[0].equalsIgnoreCase(Main.PREFIX + "disable") && args[1].equalsIgnoreCase("manualverify"))
			{
				e.getChannel().sendMessage("*disabling al's verification code...*").complete();
				e.getJDA().shutdown();
				System.exit(0);
			}
